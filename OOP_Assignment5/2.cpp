...
class transaction{
   ...
   dot(dot),memid(memid),bookid(bookid),serial(serial),issued(issued){all_transactions.push_back(*this);}
   public:
/*used in issue_book() an dreturn_book()*/
...
int main() {
  /*member* m;
  int input = 0;
  cout<<"Enter new member ? (yes=1,no=0): ";
  cin>>input;
  while(input!=0){
    cout<<"Enter student or faculty member ? (student=2,faculty=3): ";
    cin>>input;
    cin.ignore();
    string x,y;
    cout<<"Enter name: ";
